...
class Divisao_de_contas():
    ...
    def limpando_dados(self):
        self.controle = pd.read_excel(r'controle.xlsx')
        self.saldo = pd.read_excel(r'Saldo.xlsx')
        self.pl = pd.read_excel(r'PL Total.xlsx')
    ...
    def filtrando_dados_e_separando_operadores(self,arquivo_compilado):
        ...
        self.filtrando_saldo = self.arquivo_compilado.loc[self.arquivo_compilado['Saldo']>1000].sort_values(by='Saldo',ascending=False)
    ...
    def contas_nao_encontradas(self,arquivo_compilado):
        self.contas_nao_encontrados = arquivo_compilado[arquivo_compilado['Nome']==''].reset_index()
